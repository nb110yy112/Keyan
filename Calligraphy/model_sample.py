...
class ppo_agent():
    def __init__(self, ckpt_dir='./ckpt', sum_dir='./summaries',
        ...
        ratio = tf.concat([tf.expand_dims(pi[0][:, tf.cast(self.ph_a[0][0], tf.int32)]
                                          / (oldpi[0][:, tf.cast(self.ph_a[0][0], tf.int32)] + 1e-5), axis=-1),
                          pi[1].prob(self.ph_a[:, 1:]) / (oldpi[1].prob(self.ph_a[:, 1:]) + 1e-5)], axis=1)
    ...
    def _build_ph(self):
        ...
        self.ph_ls = tf.placeholder(tf.float32, [None, 64], 'last_state')
    ...
    def _build_value(self):
        with tf.variable_scope('critic'):
            with tf.variable_scope('feature'):
                ...
                cnn2 = tf.layers.conv2d(cnn1, 32, [5, 5], strides=4,
                                        activation=tf.nn.leaky_relu, name='cnn2')
                fl1 = tf.layers.flatten(cnn2, name='fl1')
        ...
        with tf.variable_scope(name):
            with tf.variable_scope('feature'):
                cnn1 = tf.layers.conv2d(self.ph_s, 32, [3, 3], strides=2, trainable=trainable,
                                        activation=tf.nn.leaky_relu, name='cnn1')
                cnn2 = tf.layers.conv2d(cnn1, 32, [5, 5], strides=4, trainable=trainable,
                                        activation=tf.nn.leaky_relu, name='cnn2')
                fl1 = tf.layers.flatten(cnn2, name='fl1')

            with tf.variable_scope('cmd'):
                dn1 = tf.layers.dense(tf.concat([fl1, self.ph_la], axis=1), 64,
    ...
    def _build_discriminator(self, img):
        with tf.variable_scope('discriminator', reuse=tf.AUTO_REUSE):
            ...
            cnn2 = tf.layers.conv2d(cnn1, 32, [5, 5], strides=4,
                                    activation=tf.nn.leaky_relu, name='cnn2')
            pool1 = tf.layers.max_pooling2d(cnn2, 3, 1, name='pool1')
    ...
    def update(self):
        global GLOBAL_UPDATE_COUNTER, Replay, count
        while not COORD.should_stop():
            if GLOBAL_EP < EP_MAX:
                ...
                for data in Replay:
                    data = np.vstack(data)
                    s_o, s, a, r, la = data[:, :S_DIM].reshape([-1, 64, 64, 1]), \
                    ...
                    [self.sess.run(self.atrain_op, {self.ph_s: s, self.ph_a: a, self.ph_adv: adv,
                                                    self.ph_la: la}) for _ in range(UPDATE_STEP)]
...
class Worker(object):
    ...
    def work(self):
        ...
        while not COORD.should_stop():
            ...
            ep_r = 0
            buffer_s, buffer_a, buffer_r, buffer_la, buffer_so = [], [], [], [],[]
            s_o = DATASET_ALL[100]
            ...
            for t in range(num):
                ...
                if t == num - 1 or GLOBAL_UPDATE_COUNTER >= MIN_BATCH_SIZE:
                    ...
                    bso, bs, ba, br, bla = np.vstack(buffer_so),\
                                           np.vstack(buffer_s),\
                                           np.vstack(buffer_a),\
                                           np.array(discounted_r),\
                                           np.vstack(buffer_la)
                    buffer_s, buffer_a, buffer_r, buffer_la, buffer_so = [], [], [], [],[]
                    QUEUE.put(np.hstack((bso, bs, ba, br, bla)))
