...
class Worker(object):
    ...
    def work(self):
        global GLOBAL_EP, GLOBAL_RUNNING_R, GLOBAL_UPDATE_COUNTER, \
            DATASET_BH, dloss, Replay_d
        while not COORD.should_stop():
            s = Image.new('L', (width, height), 255)
            s = np.expand_dims((np.array(s)*2-255)/255, axis=-1)
            s_1 = s
            for _ in range(5):
                act = np.array([np.random.randint(20, 50), np.random.randint(0, 20), np.random.randint(0, 20)])
                s_o = self.ppo.Draw(s_1, act)
                s_1 = s_o
            ep_r = 0
            ...
            for t in range(num):
                ...
                ep_r += r
                s_s.append(s)
                utils.imwrite(utils.immerge(np.array(s_s), len(s_s), 1),
                              './save_img/test_%d_%d.jpg'%(GLOBAL_UPDATE_COUNTER, t))

                GLOBAL_UPDATE_COUNTER += 1                      # count to minimum batch size
                if t == num - 1 or GLOBAL_UPDATE_COUNTER >= MIN_BATCH_SIZE:
                    buffer_r.append(r)  # normalize reward, find to be useful
                    if t == num - 1:
                        self.ppo.ntrain(s_, 5, act)
