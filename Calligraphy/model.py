...
class ppo_agent():
    def __init__(self, ckpt_dir='./ckpt', sum_dir='./summaries',
        ...
        # actor
        pi, pi_params = self._build_policy('pi', trainable=True)
        oldpi, oldpi_params = self._build_policy('oldpi', trainable=False)
        self.sample_op = tf.nn.relu(tf.squeeze(pi.sample(1), axis=0))  # choosing action
        self.update_oldpi_op = [oldp.assign(p) for p, oldp in zip(pi_params, oldpi_params)]

        ratio = pi.prob(self.ph_a) / (oldpi.prob(self.ph_a) + 1e-5)
        ...
        self.nloss += tf.reduce_mean(tf.square(self.sample_op - self.ph_la / (self.ph_la + 1e5)))
    ...
    def _build_ph(self):
        ...
        self.ph_la = tf.placeholder(tf.float32, [None, A_DIM], 'last_a')
        self.ph_a = tf.placeholder(tf.float32, [None, A_DIM], 'action')
    ...
    def _build_policy(self, name, trainable):
        with tf.variable_scope(name):
            ...
            with tf.variable_scope('cmd'):
                ...
                mu = A_BOUND * tf.layers.dense(dn1, A_DIM, tf.nn.tanh, trainable=trainable, name='mu')
    ...
    def act(self, obs):
        ...
        cmd = self.sess.run(self.sample_op, feed_dict=feed_dict)
        return np.squeeze(cmd, axis=0)
    ...
    def Draw(self, state, cmd):
        ...
        word = chr((cmd[0]).astype(np.int32) + s_bh)
        size = (cmd[0]).astype(np.int32)
        loc_x = (cmd[1]).astype(np.int32)
        loc_y = (cmd[2]).astype(np.int32)
    ...
    def update(self):
        ...
        while not COORD.should_stop():
            if GLOBAL_EP < EP_MAX:
                ...
                data = np.vstack(data)
                s, a, r = data[:, :S_DIM].reshape([-1, 64, 64, 1]), data[:, S_DIM: S_DIM + A_DIM], data[:, -1:]
                adv = self.sess.run(self.advantage, {self.ph_s: s, self.ph_dr: r})
                [self.sess.run(self.atrain_op, {self.ph_s: s, self.ph_a: a, self.ph_adv: adv}) for _ in
                 range(UPDATE_STEP)]
...
class Worker(object):
    ...
    def work(self):
        global GLOBAL_EP, GLOBAL_RUNNING_R, GLOBAL_UPDATE_COUNTER, \
            DATASET_ALL, DATASET_BH, dloss, Replay_d
        while not COORD.should_stop():
            cmd_bh = []
            for t in range(len(DATASET_BH)):
                cmd_bh.append(self.ppo.act(DATASET_BH[t]))
            cmd_bh = np.array(cmd_bh)
            cmd_bh[:, 0:3] = [50, 1, 1]
            num_bh = np.ones([cmd_bh.shape[0], 1])
            self.ppo.ntrain(DATASET_BH, num_bh, cmd_bh)
            ...
            num = self.ppo.number(s_o)
            for t in range(num):
                if not ROLLING_EVENT.is_set():                  # while global PPO is updating
                    ROLLING_EVENT.wait()                        # wait until PPO is updated
                    buffer_s, buffer_a, buffer_r = [], [], []   # clear history buffer
                a = self.ppo.act(self.ppo.connect(s_o, s))
                if a[0] >= 25 and a[0] <= 50 and a[1] < 10 and a[2] < 10:
                ...
                if t == num - 1 or GLOBAL_UPDATE_COUNTER >= MIN_BATCH_SIZE:
                    ...
                    if t == num - 1:
                        self.ppo.ntrain(s_, num)
                        Replay_d.append((s_o, s_))
                        if len(Replay_d) > 256:
                            Replay_d.pop(0)
                    if dloss < 0 and len(Replay_d) > MIN_BATCH_SIZE:
                        dloss = self.ppo.dtrain(s_o, s_)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        for test in range(100):
            s = Image.new('L', (width, height), 255)
            s = np.expand_dims((np.array(s) * 2-255)/255, axis=-1)
            s_o = DATASET_ALL[test]
            num = GLOBAL_PPO.number(s_o)
            for t in range(num):
                a = GLOBAL_PPO.act(GLOBAL_PPO.connect(s_o, s))
                s = GLOBAL_PPO.Draw(s, a)
            img.append(s)
        save_dir = './save_img'
        utils.mkdir(save_dir + '/')
        utils.imwrite(utils.immerge(np.asarray(img), 10, 10),'test.jpg')
        break
